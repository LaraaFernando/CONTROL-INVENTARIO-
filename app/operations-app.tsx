...
type Payment = { id: number; invoiceId: number; amount: number; reference: string; paidAt: string; createdBy: string; voided: number; voidedBy: string; voidedAt: string | null; voidReason: string };
...
type ClosurePreview = { businessDate: string; movementCount: number; sales: number; purchases: number; creditNotes: number; receivedPayments: number; sentPayments: number; moneyIn: number; moneyOut: number; inventoryValue: number };
...
function Suppliers({ data, post }: { data: OperationsData; post: (action: string, payload: Record<string, unknown>) => Promise<boolean> }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const saved = await post(editing ? "edit_supplier" : "add_supplier", editing ? { id: editing.id, ...payload } : payload);
    if (saved) { event.currentTarget.reset(); setOpen(false); setEditing(null); }
  }
  function close() { setOpen(false); setEditing(null); }
  function edit(supplier: Supplier) { setEditing(supplier); setOpen(true); }
  return <section className="card fill"><div className="card-head"><div><h2>Proveedores</h2><p>Condiciones comerciales, facturación y días de crédito.</p></div><button className="primary" onClick={() => { setEditing(null); setOpen(true); }}>＋ Nuevo proveedor</button></div><div className="table-wrap"><table><thead><tr><th>Proveedor</th><th>RFC</th><th>Contacto</th><th>Factura</th><th>Método</th><th>Crédito</th><th>Acciones</th></tr></thead><tbody>{data.suppliers.map(s => <tr key={s.id}><td><strong>{s.name}</strong><small>{s.businessName || "—"}</small></td><td>{s.taxId || "—"}</td><td>{s.email || s.phone || "—"}</td><td>{s.invoiceRequired ? "Sí" : "No"}</td><td><span className="pill blue">{s.defaultPaymentMethod}</span></td><td>{s.defaultPaymentMethod === "PPD" ? `${s.creditDays} días` : "Contado"}</td><td><button className="mini" onClick={() => edit(s)}>Editar</button></td></tr>)}</tbody></table>{!data.suppliers.length && <Empty text="Registra proveedores para crear pedidos y cuentas por pagar." />}</div>{open && <Modal title={editing ? `Modificar · ${editing.name}` : "Nuevo proveedor"} close={close}><form onSubmit={submit}><div className="form-grid"><Field label="Nombre *" name="name" defaultValue={editing?.name} required /><Field label="Razón social" name="businessName" defaultValue={editing?.businessName} /><Field label="RFC" name="taxId" defaultValue={editing?.taxId} /><Field label="Correo" name="email" type="email" defaultValue={editing?.email} /><Field label="Teléfono" name="phone" defaultValue={editing?.phone} /><Select label="Método predeterminado" name="defaultPaymentMethod" defaultValue={editing?.defaultPaymentMethod || "PUE"} options={[{ value: "PUE", label: "PUE · Una exhibición" }, { value: "PPD", label: "PPD · Parcialidades o diferido" }]} /><Field label="Días de crédito" name="creditDays" type="number" min="0" defaultValue={editing?.creditDays ?? 30} /><Check label="Requiere factura" name="invoiceRequired" defaultChecked={editing ? Boolean(editing.invoiceRequired) : true} /></div><Actions close={close} /></form></Modal>}</section>;
}
...
function Invoices({ data, clients, permissions, post, reload, onError }: { data: OperationsData; clients: Client[]; permissions: PermissionMap; post: (action: string, payload: Record<string, unknown>) => Promise<boolean>; reload: () => Promise<void>; onError: (message: string) => void }) {
  ...
  const warnings = useMemo(() => data.invoices
    .filter(invoice => !invoice.canceled && invoice.status !== "pagada" && daysUntil(invoice.dueDate, data.today) <= 7)
    .sort((a, b) => a.dueDate.localeCompare(b.dueDate)), [data.invoices, data.today]);
  async function create(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const payload = Object.fromEntries(new FormData(event.currentTarget));
  ...
  async function voidPayment(payment: Payment) {
    const reason = window.prompt("Motivo obligatorio de anulación del pago");
    if (reason) await post("void_payment", { paymentId: payment.id, reason });
  }
  ...
  return (
    <div className="page-stack">
      <section className="warning-grid card">
        <div className="card-head"><div><h2>Próximos vencimientos</h2><p>Cuentas por cobrar y pagar que vencen en siete días o menos.</p></div></div>
        <div className="due-list">
          {warnings.slice(0, 8).map(invoice => {
            const days = daysUntil(invoice.dueDate, data.today);
            return <div key={invoice.id}><Pill value={invoice.direction === "cliente" ? "Por cobrar" : "Por pagar"} tone={paymentTone(invoice, data.today)} /><span><strong>{invoice.folio} · {invoice.counterparty}</strong><small>{days < 0 ? `${Math.abs(days)} días vencida` : days === 0 ? "Vence hoy" : `Vence en ${days} días`}</small></span><b>{money.format(invoice.totalAmount - invoice.paidAmount)}</b></div>;
          })}
          {!warnings.length && <Empty text="No hay pagos vencidos ni próximos en los siguientes siete días." />}
        </div>
      </section>
      <section className="card fill">
        <div className="card-head"><div><h2>Facturación documental</h2><p>CFDI recibidos y enviados, PUE/PPD, saldos y archivos privados; sin timbrado SAT.</p></div><button className="primary" onClick={() => setOpen(true)}>＋ Registrar factura</button></div>
        <div className="table-wrap">
          <table>
            <thead><tr><th>Folio</th><th>Tipo</th><th>Cliente / proveedor</th><th>PUE/PPD</th><th>Vencimiento</th><th>Total</th><th>Saldo</th><th>Estatus</th><th>Pagos</th><th>Documentos</th><th>Acciones</th></tr></thead>
            <tbody>{data.invoices.map(invoice => {
              const invoicePayments = data.payments.filter(payment => payment.invoiceId === invoice.id);
              return <tr key={invoice.id} className={invoice.canceled ? "voided-row" : ""}>
                <td><code>{invoice.folio}</code><small>{invoice.uuid || "Sin UUID"}</small></td>
                <td>{invoice.direction === "cliente" ? "Cliente" : "Proveedor"}</td>
                <td>{invoice.counterparty}</td>
                <td><Pill value={invoice.paymentMethod} tone="blue" /><small>{invoice.creditDays ? `${invoice.creditDays} días` : "Contado"}</small></td>
                <td><Pill value={dateOnly(invoice.dueDate)} tone={paymentTone(invoice, data.today)} /></td>
                <td>{money.format(invoice.totalAmount)}</td>
                <td><b>{money.format(invoice.totalAmount - invoice.paidAmount)}</b></td>
                <td><Pill value={invoice.canceled ? "Cancelada" : invoice.status} tone={invoice.canceled ? "red" : invoice.status === "pagada" ? "green" : invoice.status === "parcial" ? "yellow" : "blue"} /></td>
                <td><div className="payment-list">{invoicePayments.map(payment => <span key={payment.id} className={payment.voided ? "voided-payment" : ""}><small>{money.format(payment.amount)} · {dateOnly(payment.paidAt)}</small>{payment.voided ? <Pill value="Anulado" tone="red" /> : !invoice.canceled && <button className="mini danger" onClick={() => voidPayment(payment)}>Anular</button>}</span>)}{!invoicePayments.length && "—"}</div></td>
                <td>{data.files.filter(file => file.invoiceId === invoice.id).map(file => <a className="file-link" key={file.id} href={`/api/invoice-files?id=${file.id}`}>{file.kind.toUpperCase()}</a>)}</td>
                <td><div className="row-actions">{permissions["invoices.files"] && !invoice.canceled && <button className="mini" onClick={() => setFilesFor(invoice)}>XML/PDF</button>}{!invoice.canceled && invoice.status !== "pagada" && <button className="mini" onClick={() => pay(invoice)}>Pago</button>}{invoice.direction === "cliente" && !invoice.canceled && <button className="mini" onClick={() => prepareEmail(invoice)}>Preparar envío</button>}{!invoice.canceled && <button className="mini danger" onClick={() => cancel(invoice)}>Cancelar</button>}</div></td>
              </tr>;
            })}</tbody>
          </table>
          {!data.invoices.length && <Empty text="Aún no hay facturas registradas." />}
        </div>
        {open && <Modal title="Registrar factura" close={() => setOpen(false)}><form onSubmit={create}><div className="form-grid"><label><span>Tipo *</span><select value={direction} onChange={e => setDirection(e.target.value)}><option value="cliente">Enviada a cliente</option><option value="proveedor">Recibida de proveedor</option></select></label><Field label="Folio *" name="folio" required /><Field label="UUID fiscal" name="uuid" />{direction === "cliente" ? <Select label="Cliente *" name="clientId" options={clients.map(c => ({ value: String(c.id), label: c.name }))} /> : <Select label="Proveedor *" name="supplierId" options={data.suppliers.map(s => ({ value: String(s.id), label: s.name }))} />}<label><span>Método *</span><select name="paymentMethod" value={method} onChange={e => setMethod(e.target.value)}><option value="PUE">PUE</option><option value="PPD">PPD</option></select></label>{method === "PPD" && <Field label="Días de crédito" name="creditDays" type="number" min="1" defaultValue="30" />}<Field label="Fecha de emisión" name="issueDate" type="date" defaultValue={data.today} required /><Field label="Subtotal" name="subtotal" type="number" min="0" step="0.01" /><Field label="Impuestos" name="taxAmount" type="number" min="0" step="0.01" /><Field label="Total *" name="totalAmount" type="number" min="0.01" step="0.01" required /><Field label="Notas" name="notes" wide /></div><Actions close={() => setOpen(false)} /></form></Modal>}
        {filesFor && <Modal title={`Documentos · ${filesFor.folio}`} close={() => setFilesFor(null)}><form onSubmit={upload}><div className="form-grid"><Select label="Tipo de archivo" name="kind" options={[{ value: "xml", label: "XML fiscal" }, { value: "pdf", label: "Representación PDF" }]} /><label><span>Archivo *</span><input name="file" type="file" accept=".xml,.pdf,application/xml,application/pdf" required /></label></div><p className="field-note">Máximo 10 MB. Los archivos se almacenan de forma privada y requieren sesión y permiso para descargarse.</p><Actions close={() => setFilesFor(null)} label="Cargar archivo" /></form></Modal>}
      </section>
    </div>
  );
}
...
function Closures({ data, post }: { data: OperationsData; post: (action: string, payload: Record<string, unknown>) => Promise<boolean> }) {
  ...
  return <div className="page-stack">{preview && <section className="stats closure-stats"><Stat label="Movimientos" value={String(preview.movementCount)} note={preview.businessDate} /><Stat label="Ventas registradas" value={money.format(preview.sales)} note="Operación del día" /><Stat label="Compras registradas" value={money.format(preview.purchases)} note="Operación del día" /><Stat label="Notas de crédito" value={money.format(preview.creditNotes)} note="Aplicadas en el día" /><Stat label="Dinero que entró" value={money.format(preview.moneyIn)} note="Cobros no anulados" /><Stat label="Dinero que salió" value={money.format(preview.moneyOut)} note="Pagos no anulados" /><Stat label="Inventario final" value={money.format(preview.inventoryValue)} note="A costo de adquisición" /></section>}<section className="card"><div className="card-head"><div><h2>Corte diario</h2><p>Concilia movimientos, cobros, pagos e inventario. Esta versión no genera respaldos.</p></div>{preview && !data.closures.some(c => c.businessDate === preview.businessDate) && <button className="primary" onClick={confirm}>Confirmar corte de hoy</button>}</div><div className="table-wrap"><table><thead><tr><th>Fecha operativa</th><th>Movimientos</th><th>Dinero que entró</th><th>Dinero que salió</th><th>Inventario</th><th>Cerró</th><th>Hora</th></tr></thead><tbody>{data.closures.map(closure => <tr key={closure.id}><td><code>{closure.businessDate}</code></td><td>{closure.movementCount}</td><td>{money.format(closure.moneyIn)}</td><td>{money.format(closure.moneyOut)}</td><td>{money.format(closure.inventoryValue)}</td><td>{closure.closedBy}</td><td>{dateTime(closure.createdAt)}</td></tr>)}</tbody></table>{!data.closures.length && <Empty text="Aún no se ha confirmado ningún corte." />}</div></section></div>;
}
...
function Select({ label, name, options, defaultValue = "" }: { label: string; name: string; options: { value: string; label: string }[]; defaultValue?: string }) { return <label><span>{label}</span><select name={name} required defaultValue={defaultValue}><option value="">Seleccionar…</option>{options.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>; }
