...
export async function POST(request:Request){
  try{
    ...
    const method=text(body.paymentMethod||supplier.default_payment_method).toUpperCase()==="PPD"?"PPD":"PUE";const creditDays=method==="PUE"?0:Number(body.creditDays??supplier.credit_days??30);if(method==="PPD"&&(!Number.isInteger(creditDays)||creditDays<1))throw new AuthError("Los días de crédito deben ser un entero mayor a cero.",400);
