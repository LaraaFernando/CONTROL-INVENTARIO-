...
export async function POST(request: Request) {
  try {
    ...
    const reference = text(body.reference) || `VTA-${saleDate.replaceAll("-", "")}-${Date.now().toString(36).toUpperCase()}`;
    ...
    return Response.json({ ok: true, reference, movementIds, lineCount: lines.length, totalAmount, warning }, { status: 201 });
  } catch (error) {
